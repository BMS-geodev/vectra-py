...
class LocalDocumentIndex(LocalIndex):
    ...
    async def is_catalog_created(self) -> bool:
        return (Path(self.folder_path) / 'catalog.json').exists()

    async def save_catalog(self):
        catalog_path = Path(self.folder_path) / "catalog.json"
        async with aiofiles.open(catalog_path, "w") as f:
            await f.write(json.dumps(self._catalog))
            ...
            try:
                for i, chunk in enumerate(chunks):
                    ...
                    await self.insert_item(
                        ...
                        manage_update=False  # Skip begin/end update in insert_item, we manage it here
                    )

                # Save metadata and text files
                if metadata:
    ...
    async def query_documents(self, query: str, options: DocumentQueryOptions = None) -> List[LocalDocumentResult]:
        ...
        # print('Query embeddings:', embeddings.__dict__)
        results = await self.query_items(embeddings.output, options.max_chunks, options.filter)
        document_chunks = {}
    ...
    async def load_index_data(self):
        ...
        #                                     asyncio.to_thread(await self.is_catalog_created),
        #                                     asyncio.sleep(1)
        #                                     )
        if await self.is_catalog_created():
            # Load catalog
